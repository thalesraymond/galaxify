...
<div class="relative">
  <button on:click={toggleDialog} class="text-[var(--primary)] hover:text-[var(--primary)]/80 transition-colors duration-200">
    <img src="/logo/galaxify.svg" alt="Theme switcher" class="h-8 w-8" />
  </button>

  {#if showDialog}
    <div role="dialog" class="absolute right-0 mt-2 w-64 bg-[var(--background)]/50 rounded-lg p-6 border border-[var(--foreground)]/20 shadow-lg backdrop-blur-sm">
