...
## 1. Monorepo Setup with pnpm, Next.js, and Express.js
...
- **Decision**: Use Vite for building the backend and Vitest for testing all packages. Next.js has its own build toolchain.
...
## 3. API Proxy from Next.js to Express.js

- **Decision**: Use the built-in proxy feature of Next.js. The `next.config.js` file will be configured with a `rewrites` rule to proxy requests from `/api/:path*` to the backend server.
- **Rationale**: This is the standard and simplest way to handle API proxying in Next.js during development. For production, a reverse proxy like Nginx or a cloud provider's API gateway would be used, but that is out of scope for this feature.
- **Alternatives considered**: Using a custom server in Next.js. This adds unnecessary complexity.
